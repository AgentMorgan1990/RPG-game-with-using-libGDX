...
/*
 * Класс для создания и управления предметами, выпадающими из существ
 */
...
public class ItemsEmitter {
    ...
    public ItemsEmitter() {
        ...
        regions = new TextureRegion(texture).split(40, 40)[0];
    }

    public void render(SpriteBatch batch) {
        for (int i = 0; i < items.length; i++) {
            if (items[i].isActive()) {
                batch.draw(regions[items[i].getType().index], items[i].getPosition().x, items[i].getPosition().y);
            }
        }
    }
